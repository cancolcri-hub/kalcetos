-- Kalcetos — schema aislado dentro del proyecto Supabase "digital project".
-- Aplicar en SQL editor de Supabase.
-- IMPORTANTE: tras aplicar, ir a Settings → API → Exposed schemas y añadir 'kalcetos'.

-- ============================================================
-- SCHEMA NAMESPACE
-- ============================================================

create schema if not exists kalcetos;
...
create table kalcetos.products (
  ...
  name                     text not null,
  segment                  text not null,                  -- 'adulto' | 'nino'
  brand                    text,
  ...
  -- Aviso Omnibus (UE): compare_at_price_cents solo es legalmente válido si fue el
  -- precio más bajo aplicado en los últimos 30 días. Para Kalcetos DEMO no es crítico.
...
create table kalcetos.product_variants (
  ...
  -- Si price_cents es null, se usa products.base_price_cents.
  created_at      timestamptz not null default now()
);
...
create table kalcetos.product_images (
  id              uuid primary key default gen_random_uuid(),
  product_id      uuid not null references kalcetos.products(id) on delete cascade,
  storage_path    text not null,                            -- ej: 'kalcetos-product-images/{product_id}/01.webp'
  alt_text        text not null,                            -- obligatorio para accesibilidad + SEO
...
create table kalcetos.order_items (
  ...
  unit_price_cents    integer not null check (unit_price_cents >= 0),  -- precio congelado en el momento de la compra
...
-- Permisos completos al estilo Supabase para PostgREST + service_role.
...
-- Catálogo público (solo productos activos): SELECT permitido para anon.
...
-- Customers, orders, order_items: solo accesibles vía service_role (server-side).
-- No se crean políticas para anon/authenticated → bloqueado por defecto.
-- Las operaciones de escritura/lectura ocurren en /api/stripe/webhook y demás API routes
-- usando el cliente server con SUPABASE_SERVICE_ROLE_KEY (bypass RLS).

-- ============================================================
-- STORAGE BUCKET (manual desde dashboard)
-- ============================================================

-- Crear desde el dashboard Supabase:
--   bucket: kalcetos-product-images
--   public: true (lectura pública)
--   file size limit: 10 MB
--   allowed mime types: image/webp, image/jpeg, image/png
--
-- El nombre incluye el prefijo 'kalcetos-' para no colisionar con buckets de otros
-- tenants (clínica, futuros clientes) en el mismo proyecto Supabase "digital project".
